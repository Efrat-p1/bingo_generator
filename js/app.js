...
const unsplashLoading = document.getElementById('unsplash-loading');
const unsplashMessage = document.getElementById('unsplash-message');
...
const translations = {
    en: {
        ...
        unsplashTitle: "🔍 Or search Unsplash for images",
        unsplashText: "Or search Unsplash for images",
        unsplashPlaceholder: "e.g. dog, cat, sun...",
        btnSearch: "Search",
    ...
    he: {
        ...
        unsplashTitle: "🔍 או חפשו תמונות ב-Unsplash",
        unsplashText: "או חפשו תמונות ב-Unsplash",
        unsplashPlaceholder: "למשל: כלב, חתול, שמש...",
        btnSearch: "חפש",
        unsplashLoading: "מחפש ב-Unsplash..."
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Unsplash Search Event Listener
    if (btnUnsplashSearch && unsplashQuery) {
        btnUnsplashSearch.addEventListener('click', handleUnsplashSearch);
...
async function handleUnsplashSearch() {
    ...
    unsplashResults.style.display = 'none';
    unsplashMessage.style.display = 'none';
    unsplashResults.innerHTML = '';
    btnUnsplashSearch.disabled = true;
    ...
    try {
        ...
        if (isHebrew) {
            ...
            if (englishTranslation && englishTranslation.toLowerCase() !== finalQuery.toLowerCase()) {
        ...
        const response = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(finalQuery)}&per_page=12&orientation=portrait&client_id=${apiKey}`);
        if (!response.ok) {
        ...
        photos.forEach(photo => {
            ...
            imgWrap.addEventListener('click', async () => {
                ...
                try {
                    ...
                    alert(currentLanguage === 'he' ? 'שגיאה בהורדת התמונה' : 'Error downloading image');
                } finally {
        ...
        btnUnsplashSearch.disabled = false;
    }
}
